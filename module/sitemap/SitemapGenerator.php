...
class SitemapGenerator {
	...
	public function GetUrl ($url) {
		$agent = "Mozilla/5.0 (compatible; iProDev PHP XML Sitemap Generator/" . self::VERSION . ", http://iprodev.com)";
	...
	public function Scan ($url) {
		// global $start_url, $scanned, $pf, $this->extension, $this->skip, $this->freq, $this->priority;
		$scanned = array ();
		echo $url . self::NL;
		...
		foreach ($a1 as $val) {
			...
			if (substr ($next_url, 0, strlen ($this->start_url)) == $this->start_url) {
				$ignore = false;
				...
				if (!$ignore) {
					foreach ($this->extension as $ext) {
						if (strpos ($next_url, $ext) > 0) {
							$pr = number_format ( round ( $this->priority / count ( explode( "/", trim ( str_ireplace ( array ("http://", "https://"), "", $next_url ), "/" ) ) ) + 0.5, 3 ), 1 );
							fwrite ($pf, "  <url>\n" .
										 "    <loc>" . htmlentities ($next_url) ."</loc>\n" .
										 "    <changethis->freq>$this->freq</changethis->freq>\n" .
										 "    <this->priority>$pr</this->priority>\n" .
										 "  </url>\n");
							Scan ($next_url);
						}
					}
	...
	public function generate() {
		...
		if (!$pf) {
			echo "Cannot create $this->file!" . NL;
			return;
		}
	
		$start_url = filter_var ($this->start_url, FILTER_SANITIZE_URL);
		...
		fwrite ($pf, "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" .
					 ...
					 "  <url>\n" .
					 "    <loc>" . htmlentities ($this->start_url) ."</loc>\n" .
					 "    <changethis->freq>$this->freq</changethis->freq>\n" .
					 "    <this->priority>$this->priority</this->priority>\n" .
					 "  </url>\n");
	
		$scanned = array ();
		$this->Scan($this->start_url);
	
		fwrite ($pf, "</urlset>\n");
		fclose ($pf);
	
		echo "Done." . self::NL."$this->file created." . self::NL;
	}
}
?>